...
(defmethod report :around ((key (eql :room)) association stream format)
  (unless (or (listp (cdr association)) (null (cdr association)))
    (let ((room (cdr association)))
      (let ((m.room.name (jsown:val-safe (cl-matrix:room-state room  "m.room.name")
                                         "name"))
            (m.room.avatar (jsown:val-safe (cl-matrix:room-state room "m.room.avatar")
                                           "url")))
  ...
  (symbol-macrolet ((report-room-info?
    ...
    (let ((room-info (assoc :room (cdr association)))
      ...
      (let ((child-report
        ...
        (when report-room-info?
          (setf child-report (ensure-left-margin 4 child-report)))
