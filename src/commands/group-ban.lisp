...
(define-step room-ban (target target-user reason)
...
(define-step ban (control group sender target-user reason &rest targets)
  (unless (has-power-p control sender "ban")
    (error 'luna-permission-error :description
           (format nil "~a doesn't have permission to ban in this room.")))

  (remove-if-not (lambda (f) (typep f 'error))
   (mapgroup (luna-lambda (r)
                (room-ban r target-user reason))
              control group)))
...
(define-command-parser ban (name rest room-id event)
  ...
  (cl-ppcre:register-groups-bind (group-name target-user reason)
      ("^(\\S+)\\s+(\\S+)\\s+(.+)?" rest)
    (when (and target-user reason)
      (apply #'ban room-id group-name (jsown:val event "sender") targets))))
